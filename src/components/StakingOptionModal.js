...
const StakingOptionModal = (props) => {
    ...
    useEffect(() => {
        ...
        // setBtnDisabled(false)
    }, [stakeAmount, pending, props.amount, props.ethBalance, props.allowance1])
        ...
        try {
            ...
            try {
                ...
                dataApprove = {
                    ...
                    // args: [stakingContractAddress, props.amount],
                }
            }
    ...
    const handleStake = async () => {
        setPending(true);
        try {
            console.log("fly_modaloption_10", modalOption)
            ...
            if (modalOption === 10) {
                ...
                dataStaking = {
                    ...
                    args: [parseUnits(stakeAmount, global.EthDecimals), 20]
                }
                const preparedData = await prepareWriteContract(dataStaking)
                const writeData = await writeContract(preparedData)
                const txPendingData = waitForTransaction(writeData)
            ...
            } else if (modalOption === 30) {
                dataStaking = {
                    ...dataStaking,
                    address: stakingContractAddress,
                    abi: StakingContractABI,
                    functionName: 'stake',
                    args: [parseUnits(stakeAmount, global.EthDecimals), 30]
              ...
              dataStaking = {
                  ...
                  abi: StakingContractABI,
                  functionName: 'stake',
                  args: [parseUnits(stakeAmount, global.EthDecimals), 40]
                            ...
                            <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                                <div className="w-full flex h-full items-start justify-center p-4 text-center sm:items-start sm:p-0 font">
                                    <div className="relative text-left transition-all transform rounded-lg shadow-xl w-full my-auto sm:max-w-xl p-5 sm:px-5 bg-modal border border-gray-1 border-opacity-30">
                                        <div className="relative flex items-start justify-start w-full mb-4" >
                                        ...
                                        <div className="flex flex-col pb-3">
                                            <div className="mt-">
                                                ...
                                                <div className="flex lg:flex-row flex-col lg:items-center item-start gap-5 w-full">
                                                  <div className="flex col-12 space-x-1">
                                                        <button onClick={() => { setModalOption(10) }} className={clsx("md:p-4 p-2.5 flex flex-col items-center justify-center w-full  rounded-2xl gap-1.5 border border-gray-1 border-opacity-30",  modalOption === 10 ? "bg-violet-1 text-white" : "bg-violet-4 text-violet-2")}>
                                                            <h1 className="text-base font-bold">10 Days Lockup</h1>
                                                            <h2><span className="text-xl font-bold">10%</span><span className="ml-2 text-sm font-bold">APR</span></h2>
                                                        </button>
                                                        <button onClick={() => setModalOption(20)} className={clsx("md:p-4 p-2.5 flex flex-col items-center justify-center w-full rounded-2xl gap-1.5 border border-gray-1 border-opacity-30", modalOption === 20 ? "bg-violet-1 text-white" : "bg-violet-4 text-violet-2")}>
                                                            <h1 className="text-base font-bold">20 Days Lockup</h1>
                                                            <h2><span className="text-xl font-bold">20%</span><span className="ml-2 text-sm font-bold">APR</span></h2>
                                                        </button>
                                                        <button onClick={() => setModalOption(30)} className={clsx("md:p-4 p-2.5 flex flex-col items-center justify-center w-full rounded-2xl gap-1.5 border border-gray-1 border-opacity-30", modalOption === 30 ? "bg-violet-1 text-white" : "bg-violet-4 text-violet-2")}>
                                                            <h1 className="text-base font-bold">30 Days Lockup</h1>
                                                            <h2><span className="text-xl font-bold">30%</span><span className="ml-2 text-sm font-bold">APR</span></h2>
                                                        </button>
                                                        <button onClick={() => setModalOption(40)} className={clsx("md:p-4 p-2.5 flex flex-col items-center justify-center w-full rounded-2xl gap-1.5 border border-gray-1 border-opacity-30", modalOption === 40 ? "bg-violet-1 text-white" : "bg-violet-4 text-violet-2")}>
                                                            <h1 className="text-base font-bold">40 Days Lockup</h1>
